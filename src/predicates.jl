...
function orient2dadapt(
    ...
    pc::Point{T},
    detsum,
    ccwerrboundB::Float64=DeviceLayout.ccwerrboundB,
    ccwerrboundC::Float64=DeviceLayout.ccwerrboundC,
    resulterrbound::Float64=DeviceLayout.resulterrbound
) where {T}
    z = zero(T) * zero(T)
    ...
    @Two_Two_Diff(detleft, detlefttail, detright, detrighttail, B3, B[3], B[2], B[1])
    B[4] = B3
...
function orientation(
    ...
    pb::Point{T},
    pc::Point{T},
    ccwerrboundA::Float64=DeviceLayout.ccwerrboundA
) where {T <: Float64Like}
